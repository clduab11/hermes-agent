...
def run_full_system(host: str = None, port: int = None, reload: bool = None):
...
def run_dashboard_only(host: str = None, port: int = None, reload: bool = None):
...
Examples:
  ...
  %(prog)s --no-reload              # Disable auto-reload
    ...
    parser.add_argument(
        "--host", type=str, help="Host to bind to (default: from config)"
    )
    ...
    if args.dashboard_only:
        run_dashboard_only(host=args.host, port=args.port, reload=reload)
    else:
        run_full_system(host=args.host, port=args.port, reload=reload)
