...
from .voice_pipeline import VoiceInteraction, VoicePipeline
...
class VoiceWebSocketHandler:
    ...
    def __init__(
        self, voice_pipeline: VoicePipeline, jwt_handler: JWTHandler | None = None
    ):
        self.voice_pipeline = voice_pipeline
        self.jwt_handler = jwt_handler or JWTHandler()
        self.active_connections: Dict[str, ConnectionInfo] = {}
        self.event_streaming = (
            voice_pipeline.event_streaming
        )  # Get event streaming from pipeline
        ...
        try:
            ...
            if message_type in self._message_handlers:
                ...
                await self._send_error(
                    session_id, f"Unknown message type: {message_type}"
                )
        ...
        try:
            ...
            if interaction.audio_output:
                await self._send_audio_response(
                    session_id, interaction.audio_output.audio_data
                )
    ...
    async def _handle_ping(self, session_id: str, message: Dict[str, Any]) -> None:
        ...
        await self._send_message(
            session_id, {"type": "pong", "timestamp": message.get("timestamp")}
        )
    ...
    async def _send_audio_response(self, session_id: str, audio_data: bytes) -> None:
        ...
        try:
            ...
            await websocket.send_text(
                json.dumps({"type": "audio_response", "size": len(audio_data)})
            )
    ...
    async def _send_error(self, session_id: str, error_message: str) -> None:
        ...
        """
        await self._send_message(
            session_id, {"type": "error", "message": error_message}
        )
