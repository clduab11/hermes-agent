...
class DatabaseOptimizer:
    ...
    async def cache_conversation(self, conversation: ConversationCache) -> bool:
        ...
        try:
            cache_key = (
                f"conversation:{conversation.tenant_id}:{conversation.conversation_id}"
            )
            ...
            await asyncio.to_thread(
                self.redis_client.hset, cache_key, mapping=conversation_data
            )
            await asyncio.to_thread(
                self.redis_client.expire, cache_key, conversation.ttl_seconds
            )
            ...
            logger.debug(
                f"Cached conversation {conversation.conversation_id} for tenant {conversation.tenant_id}"
            )
            return True
    ...
    async def get_cached_conversation(
        ...
        try:
            ...
            conversation_data = await asyncio.to_thread(
                self.redis_client.hgetall, cache_key
            )
            ...
            logger.debug(
                f"Retrieved cached conversation {conversation_id} for tenant {tenant_id}"
            )
            return conversation
    ...
    async def evict_expired_conversations(self) -> int:
        ...
        try:
            ...
            keys = await asyncio.to_thread(self.redis_client.keys, "conversation:*")
            ...
            for key in keys:
                try:
                    ...
                    last_accessed = datetime.fromisoformat(
                        conversation_data.get("last_accessed", "")
                    )
    ...
    async def get_tenant_performance_metrics(
        self, tenant_id: str
    ...
    async def get_system_performance_metrics(self) -> Dict[str, Any]:
        ...
        total_conversations = sum(
            m.total_conversations for m in self.tenant_metrics.values()
        )
        ...
        if self.tenant_metrics:
            avg_response_time = sum(
                m.avg_response_time_ms for m in self.tenant_metrics.values()
            ) / len(self.tenant_metrics)
                ...
                for tenant_id, metrics in self.tenant_metrics.items():
                    if self.redis_client:
                        ...
                        keys = await asyncio.to_thread(
                            self.redis_client.keys, f"conversation:{tenant_id}:*"
                        )
