...
class KnowledgeIntegrator:
    ...
    async def learn_from_interaction(self, interaction: LearningInteraction):
        ...
        entities = await self._extract_entities(interaction.query + " " + interaction.response)
        concepts = await self._extract_legal_concepts(interaction.query + " " + interaction.response)
        
        # Update knowledge graph based on extraction
        for entity_name, entity_type in entities.items():
            await self._update_or_create_entity(entity_name, entity_type, interaction.tenant_id)
    ...
    async def search_legal_knowledge(self, query: str, tenant_id: str, limit: int = 10) -> Dict[str, Any]:
        ...
        internal_results = await self._search_internal_knowledge(query, tenant_id, limit)
        ...
        search_results["results"] = await self._rank_and_deduplicate_results(search_results["results"])
        search_results["total_results"] = len(search_results["results"])
        
        end_time = datetime.utcnow()
        search_results["search_time_ms"] = (end_time - start_time).total_seconds() * 1000
        
        logger.info(f"Legal knowledge search completed: {search_results['total_results']} results in {search_results['search_time_ms']}ms")
        return search_results
        
    async def get_contextual_knowledge(self, conversation_id: str, tenant_id: str) -> Dict[str, Any]:
                    ...
                    if related_entity_id not in conversation_entities:
                        ...
                        if related_entity.entity_type == EntityType.LEGAL_CONCEPT:
                            ...
                            context["applicable_precedents"].append({
                                "name": related_entity.name,
                                "citation": related_entity.attributes.get("citation", ""),
    ...
    async def generate_knowledge_insights(self, tenant_id: str) -> Dict[str, Any]:
        ...
        tenant_entities = [e for e in self.entities.values() 
                          if tenant_id in e.metadata.get("tenant_ids", [])]
        
        sorted_entities = sorted(tenant_entities, 
        ...
        low_confidence_entities = [e for e in tenant_entities 
        ...
        if self.learning_interactions:
            recent_interactions = [i for i in self.learning_interactions 
                                 if (datetime.utcnow() - i.timestamp).days <= 7
                                 and i.tenant_id == tenant_id]
            
            positive_feedback = len([i for i in recent_interactions 
                                   if i.user_feedback and "positive" in i.user_feedback.lower()])
            
            if recent_interactions:
                insights["learning_effectiveness"] = positive_feedback / len(recent_interactions)
                
        # Generate recommendations
        if insights["learning_effectiveness"] < 0.7:
            insights["recommendations"].append("Consider improving response quality through additional training")
            
        if len(insights["knowledge_gaps"]) > 3:
            insights["recommendations"].append("Focus on filling knowledge gaps in key legal areas")
    ...
    async def _update_or_create_entity(self, name: str, entity_type: EntityType, tenant_id: str):
    ...
    async def _infer_relationships(self, entities: Dict[str, EntityType], conversation_id: str):
        ...
        for i, entity1 in enumerate(entity_ids):
            for entity2 in entity_ids[i+1:]:
                ...
                entity1_obj = next((e for e in self.entities.values() if e.name == entity1), None)
                entity2_obj = next((e for e in self.entities.values() if e.name == entity2), None)
    ...
    def _determine_relationship_type(self, type1: EntityType, type2: EntityType) -> Optional[RelationType]:
    ...
    async def _find_related_entities(self, entity_id: str, max_depth: int = 2) -> Set[str]:
    ...
    async def _search_internal_knowledge(self, query: str, tenant_id: str, limit: int) -> List[Dict[str, Any]]:
        """Search internal knowledge graph."""
        results = []
        ...
        for entity in self.entities.values():
            if (query_lower in entity.name.lower() and 
                (tenant_id in entity.metadata.get("tenant_ids", []) or 
                 entity.entity_type == EntityType.LEGAL_CONCEPT)):
    ...
    async def _search_external_legal_sources(self, query: str, limit: int) -> List[Dict[str, Any]]:
    ...
    async def _rank_and_deduplicate_results(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _continuous_learning_loop(self):
        ...
        while True:
            try:
                ...
                recent_interactions = [i for i in self.learning_interactions 
                                     if (datetime.utcnow() - i.timestamp).total_seconds() <= 86400]  # 24 hours
                
                if recent_interactions:
                    logger.info(f"Processing {len(recent_interactions)} recent interactions for learning")
                    ...
                    await self._strengthen_relationships_from_interactions(recent_interactions)
    ...
    async def _update_entity_confidence_scores(self, interactions: List[LearningInteraction]):
        ...
        for interaction in interactions:
            for entity_id in interaction.entities_mentioned:
                if entity_id in self.entities:
                    ...
                    entity.confidence_score = min(1.0, entity.confidence_score + confidence_boost)
                    
    async def _strengthen_relationships_from_interactions(self, interactions: List[LearningInteraction]):
        ...
        for interaction in interactions:
            ...
            for i, entity1_id in enumerate(entities):
                for entity2_id in entities[i+1:]:
                    ...
                    for relationship in self.relationships.values():
                        if ((relationship.from_entity_id == entity1_id and relationship.to_entity_id == entity2_id) or
                            (relationship.from_entity_id == entity2_id and relationship.to_entity_id == entity1_id)):
                            ...
                            if interaction.conversation_id not in relationship.evidence:
                                relationship.evidence.append(interaction.conversation_id)
