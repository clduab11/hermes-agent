...
class VoiceContextManager:
    ...
    def __init__(self, redis_client=None, legal_nlp_processor=None):
        ...
        self.emotion_patterns = {
            EmotionalState.FRUSTRATED: {
                ...
                'speech_indicators': {'speaking_rate': (1.2, 2.0), 'volume': (0.7, 1.0)},
                'phrase_patterns': ['why is this', 'this is ridiculous', 'i cannot believe']
            },
            EmotionalState.ANXIOUS: {
                'keywords': ['worried', 'concerned', 'nervous', 'scared', 'anxious'],
                'speech_indicators': {'speaking_rate': (0.5, 0.9), 'pitch_variance': (0.2, 1.0)},
                'phrase_patterns': ['what if', 'i am worried about', 'this makes me nervous']
            },
            EmotionalState.URGENT: {
                'keywords': ['urgent', 'emergency', 'immediately', 'asap', 'right now'],
                'speech_indicators': {'speaking_rate': (1.3, 2.5), 'volume': (0.8, 1.0)},
                'phrase_patterns': ['need this now', 'this is urgent', 'emergency situation']
            },
            EmotionalState.SATISFIED: {
                'keywords': ['great', 'perfect', 'excellent', 'wonderful', 'satisfied'],
                'speech_indicators': {'tone_positivity': (0.6, 1.0), 'speaking_rate': (0.8, 1.2)},
                'phrase_patterns': ['that works', 'sounds good', 'perfect']
            },
            EmotionalState.CONFUSED: {
                'keywords': ['confused', 'unclear', 'do not understand', 'what does that mean'],
                'speech_indicators': {'pause_frequency': (0.3, 1.0), 'pitch_variance': (0.1, 0.4)},
                'phrase_patterns': ['i do not understand', 'can you explain', 'what does that mean']
            }
        }
        
        # Legal conversation phase indicators
        self.phase_indicators = {
            ConversationPhase.GREETING: ['hello', 'hi', 'good morning', 'good afternoon'],
            ConversationPhase.INTAKE: ['need help with', 'have a legal issue', 'looking for lawyer'],
            ConversationPhase.FACT_GATHERING: ['what happened', 'when did', 'where did', 'details'],
            ConversationPhase.ADVICE_SEEKING: ['what should i do', 'what are my options', 'legal advice'],
            ConversationPhase.SCHEDULING: ['appointment', 'meeting', 'when can', 'schedule'],
            ConversationPhase.EMERGENCY: ['emergency', 'urgent', 'arrested', 'court tomorrow']
        }
    ...
    async def update_conversation_context(
        ...
        if text_input:
            ...
            if new_phase != context.conversation_phase:
                logger.info(f"Session {session_id} phase changed: {context.conversation_phase} -> {new_phase}")
            ...
            for emotion, patterns in self.emotion_patterns.items():
                if 'speech_indicators' in patterns:
                    ...
                    if emotion in emotion_scores:
                        emotion_scores[emotion] = (emotion_scores[emotion] + audio_score) / 2
                    else:
    ...
    async def calculate_urgency_level(self, context: ConversationContext) -> int:
        ...
        # Emotional factors
        if context.emotional_state.primary_emotion in [EmotionalState.URGENT, EmotionalState.FRUSTRATED]:
            base_urgency += 2
        elif context.emotional_state.primary_emotion in [EmotionalState.ANXIOUS, EmotionalState.ANGRY]:
            base_urgency += 1
    ...
    async def detect_confidential_content(self, text: str) -> Set[str]:
        ...
        confidential_indicators = {
            'attorney_client': ['attorney-client', 'privileged', 'confidential communication'],
    ...
    async def get_contextual_suggestions(
        ...
        return sorted(suggestions, key=lambda x: {'critical': 4, 'high': 3, 'medium': 2, 'low': 1}[x['priority']], reverse=True)
    ...
    async def cleanup_expired_memories(self, session_id: str) -> None:
        ...
        logger.debug(f"Cleaned up {len(expired_keys)} expired memories for session {session_id}")
    ...
    async def end_session_context(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        summary = {
            'session_id': session_id,
            'duration': (datetime.utcnow() - context.last_updated).total_seconds() / 60,  # minutes
    ...
    async def store_context_redis(self, context: ConversationContext) -> None:
        ...
        try:
            ...
            emotional_state['primary_emotion'] = emotional_state['primary_emotion'].value if hasattr(emotional_state['primary_emotion'], 'value') else str(emotional_state['primary_emotion'])
            emotional_state['secondary_emotions'] = [e.value if hasattr(e, 'value') else str(e) for e in emotional_state['secondary_emotions']]
    ...
    async def get_session_analytics(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'emotional_progression': [em.primary_emotion.value for em in emotion_history],
            'average_emotional_intensity': sum(em.emotional_intensity for em in emotion_history) / len(emotion_history) if emotion_history else 0,
            'stress_indicators_detected': sum(len(em.stress_indicators) for em in emotion_history),
            'urgency_level': context.urgency_level,
            ...
            'session_duration_minutes': (datetime.utcnow() - context.last_updated).total_seconds() / 60
        }
