...
class LegalNLPProcessor:
    ...
    async def analyze_text(self, text: str) -> LegalAnalysis:
        ...
        try:
            ...
            logger.debug(
                f"Legal analysis completed: {len(entities)} entities, {len(concepts)} concepts, risk={risk_level}"
            )
            return analysis
    ...
    def _extract_legal_entities(self, text: str) -> List[LegalEntity]:
        ...
        try:
            ...
            citation_pattern = (
                r"\b\d+\s+[A-Z][a-z.]+\s+\d+(?:\s*\([A-Z][a-z.]*\s+\d{4}\))?"
            )
                ...
                for match in re.finditer(pattern, text, re.IGNORECASE):
                    entities.append(
                        LegalEntity(
                            ...
                            end_pos=match.end(),
                            metadata={
                                "concept_category": self._categorize_legal_concept(term)
                            },
                        )
                    )
    ...
    def _analyze_contract_concepts(
        self, text: str, entities: List[LegalEntity]
    ) -> List[LegalConcept]:
        ...
        if re.search(r"\b(?:offer|propose|accept|agree)\b", text, re.IGNORECASE):
            concepts.append(
                LegalConcept(
                    ...
                    compliance_notes=[
                        "Ensure all elements of contract formation are present"
                    ],
                )
            )
        ...
        monetary_entities = [
            e for e in entities if e.entity_type == LegalEntityType.MONETARY_AMOUNT
        ]
        if monetary_entities or re.search(
            r"\b(?:consideration|payment|exchange)\b", text, re.IGNORECASE
        ):
            concepts.append(
                LegalConcept(
    ...
    def _analyze_procedural_concepts(
        self, text: str, entities: List[LegalEntity]
    ) -> List[LegalConcept]:
        ...
        if re.search(
            r"\b(?:deadline|statute of limitations|time limit)\b", text, re.IGNORECASE
        ):
            concepts.append(
                LegalConcept(
    ...
    def _analyze_compliance_concepts(
        self, text: str, entities: List[LegalEntity]
    ) -> List[LegalConcept]:
        ...
        if re.search(r"\b(?:regulation|compliance|violation)\b", text, re.IGNORECASE):
            concepts.append(
                LegalConcept(
                    ...
                    requires_attorney=True,
                    compliance_notes=[
                        "Review applicable regulations and compliance requirements"
                    ],
                )
            )
    ...
    def _assess_risk_level(
        self, text: str, entities: List[LegalEntity], concepts: List[LegalConcept]
    ) -> LegalRiskLevel:
    ...
    def _calculate_compliance_score(
        ...
        try:
            ...
            high_risk_concepts = [
                c
                for c in concepts
                if c.risk_level in [LegalRiskLevel.HIGH, LegalRiskLevel.CRITICAL]
            ]
            ...
            if re.search(
                r"\b(?:you should|i recommend|my advice|legal opinion)\b",
                text,
                re.IGNORECASE,
            ):
                score -= 25
            ...
            if (
                ...
                or "consult an attorney" in text.lower()
            ):
                score += 10
    ...
    def _requires_human_review(
        self, risk_level: LegalRiskLevel, compliance_score: float
    ) -> bool:
        ...
        if (
            compliance_score
            < self.compliance_thresholds["attorney_review_threshold"] * 100
        ):
            return True
    ...
    def _generate_disclaimers(
        self, concepts: List[LegalConcept], risk_level: LegalRiskLevel
    ) -> List[str]:
    ...
    def _generate_recommendations(
        ...
        time_sensitive_entities = [
            e
            for e in entities
            if e.entity_type in [LegalEntityType.DATE, LegalEntityType.DEADLINE]
        ]
        ...
        for concept in concepts:
            if concept.requires_attorney:
                recommendations.append(
                    f"Seek legal advice regarding {concept.concept.lower()}"
                )
