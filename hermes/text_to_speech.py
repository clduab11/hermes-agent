...
class KokoroTTS:
    ...
    async def _health_check(self) -> None:
        ...
        try:
            ...
            raise ConnectionError(
                f"Kokoro TTS API returned error: {e.response.status_code}"
            )
    ...
    async def synthesize_text(
        ...
        if not self._client:
            raise RuntimeError(
                "Kokoro TTS client not initialized. Call initialize() first."
            )
        ...
        try:
            ...
            sample_rate = int(
                response.headers.get("X-Sample-Rate", settings.sample_rate)
            )
    ...
    def _add_legal_disclaimer(self, text: str) -> str:
        ...
        contains_legal_content = any(
            keyword in text_lower for keyword in legal_keywords
        )
