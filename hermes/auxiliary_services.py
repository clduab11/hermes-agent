...
class AuditLoggingSubscriber:
    ...
    async def _process_audit_event(self, event: VoiceEvent):
        ...
        try:
            ...
            if (
                len(self.audit_logs) >= self._batch_size
                or (datetime.now() - self._last_flush).seconds >= self._flush_interval
            ):
...
class PerformanceMonitoringSubscriber:
    ...
    async def _process_performance_event(self, event: VoiceEvent):
        ...
        try:
            ...
            if event.event_type == EventType.VOICE_INTERACTION_STARTED:
                ...
                total_time = (
                    event.data.get("total_processing_time", 0) * 1000
                )  # Convert to ms
                ...
                if session_id in self.session_timings:
                    ...
                    if "start_time" in session_data:
                        end_to_end = (
                            event.timestamp.timestamp() - session_data["start_time"]
                        ) * 1000
                        self.performance_metrics["end_to_end_latency"].append(
                            end_to_end
                        )
    ...
    async def _alert_performance_issue(
        self, session_id: str, latency_ms: float, event: VoiceEvent
    ):
        """Alert on performance issues."""
        alert_key = (
            f"{event.tenant_id}:{int(latency_ms // 50) * 50}"  # Group by 50ms buckets
        )
        ...
        if alert_key not in self.performance_alerts_sent:
            ...
            logger.warning(
                f"Performance alert: {latency_ms:.2f}ms latency in session {session_id}"
            )
