...
from ..integrations.clio.token_repository import (
    delete_clio_tokens,
    get_clio_tokens,
    upsert_clio_tokens,
)
...
async def get_clio_client(
    current_user=Depends(get_current_user), tenant_context=Depends(get_tenant_context)
) -> ClioClient:
    ...
    async with SecureAsyncSession(db_session) as session:
        tokens = await get_clio_tokens(
            session, tenant_context.tenant_id, current_user["id"]
        )
        if not tokens:
            raise HTTPException(
                status_code=401, detail="Clio not authorized for this tenant/user"
            )
...
async def handle_oauth_callback_get(
    ...
    try:
        ...
        async with SecureAsyncSession(db_session) as session:
            await upsert_clio_tokens(
                session, tenant_context.tenant_id, current_user["id"], tokens
            )
...
async def list_contacts(
    ...
    try:
        contacts = await client.list_contacts(limit=limit, offset=offset, search=search)
        return contacts
    except Exception as e:
...
async def update_contact(
    ...
    try:
        updated_contact = await client.update_contact(
            contact_id, contact_update.model_dump(exclude_none=True)
        )
...
async def update_matter(
    ...
    try:
        updated_matter = await client.update_matter(
            matter_id, matter_update.model_dump(exclude_none=True)
        )
...
async def list_documents(
    ...
    try:
        documents = await client.list_documents(
            limit=limit, offset=offset, matter_id=matter_id, contact_id=contact_id
        )
        return documents
    except Exception as e:
...
async def bulk_import_contacts(
    ...
    try:
        ...
        background_tasks.add_task(
            client.bulk_import_contacts, [contact.model_dump() for contact in contacts]
        )
        return {
            "status": "started",
...
@router.get("/health")
async def clio_health_check(client: ClioClient = Depends(get_clio_client)):
