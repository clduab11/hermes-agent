...
async def get_dashboard_overview(
    ...
    try:
        ...
        call_stats = await analytics_engine.get_call_statistics(
            tenant_id=tenant_context.tenant_id, time_range=time_range
        )
...
async def get_call_volume_chart(
    ...
    try:
        if not (settings.demo_mode or settings.debug):
            raise HTTPException(
                status_code=503,
                detail="Analytics charts unavailable until data is ready",
            )
        # Mock chart data
        chart_data = {
        ...
        return {"status": "success", "data": chart_data, "time_range": time_range.value}

    except Exception as e:
        logger.error(f"Failed to get call volume chart: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to retrieve call volume chart"
        )
...
async def get_revenue_chart(
    ...
    try:
        if not (settings.demo_mode or settings.debug):
            raise HTTPException(
                status_code=503, detail="Revenue chart unavailable until data is ready"
            )
...
async def get_recent_activity(
    ...
    try:
        if not (settings.demo_mode or settings.debug):
            raise HTTPException(
                status_code=503,
                detail="Recent activity unavailable until data is ready",
            )
        ...
        activities = [
            ...
            },
            {
                ...
                "user": "Integration",
                "details": {"synced_records": 45, "status": "success"},
            },
        ]
...
async def get_system_performance(
    ...
    try:
        if not (settings.demo_mode or settings.debug):
            raise HTTPException(
                status_code=503,
                detail="System performance data unavailable until data is ready",
            )
        ...
        performance_data = {
            ...
            "resource_utilization": {"cpu": 32.5, "memory": 48.2, "disk": 67.1},
            "integrations": {
                "clio": "connected",
                "mcp_servers": 8,
                "healthy_servers": 8,
...
async def get_voice_system_status(
    ...
    try:
        ...
        return {"status": "success", "data": voice_status}
